...
<link href="https://fonts.googleapis.com/css2?family=Playfair+Display:ital,wght@0,400;0,600;0,700;1,400;1,700&family=Cinzel:wght@400;600;700&family=IM+Fell+English:ital@0;1&family=Inter:wght@300;400;500;600;700&display=swap" rel="stylesheet">
...
/* ── Keyframes ──────────────────────────────────────────── */
@keyframes sReveal    { from{opacity:0;transform:translateY(24px)} to{opacity:1;transform:translateY(0)} }
@keyframes ewDot      { 0%,100%{opacity:1;transform:scale(1)} 50%{opacity:.28;transform:scale(.46)} }
@keyframes phRing     { 0%,100%{transform:translate(-50%,-50%) scale(1);opacity:.18} 50%{transform:translate(-50%,-50%) scale(1.12);opacity:.06} }
@keyframes haloGlow   { 0%,100%{box-shadow:0 0 28px rgba(201,162,39,.22),0 0 60px rgba(201,162,39,.08)} 50%{box-shadow:0 0 50px rgba(201,162,39,.42),0 0 100px rgba(201,162,39,.16)} }
@keyframes shimmer    { 0%{opacity:0;transform:translateX(-100%) skewX(-18deg)} 60%{opacity:.5} 100%{opacity:0;transform:translateX(220%) skewX(-18deg)} }
@keyframes floatOrb   { 0%,100%{transform:translateY(0) scale(1)} 50%{transform:translateY(-22px) scale(1.06)} }
@keyframes statPulse  { 0%,100%{box-shadow:0 0 30px rgba(201,162,39,.08),inset 0 1px 0 rgba(255,255,255,.04)} 50%{box-shadow:0 0 60px rgba(201,162,39,.18),inset 0 1px 0 rgba(255,255,255,.04)} }
@keyframes accentGrow { from{transform:scaleY(0)} to{transform:scaleY(1)} }
@keyframes burnGlow   { 0%,100%{opacity:.40;transform:scale(1)} 40%{opacity:.62;transform:scale(1.08)} 70%{opacity:.34;transform:scale(.96)} }
@keyframes scriptureSlide { from{opacity:0;transform:translateY(8px)} to{opacity:1;transform:translateY(0)} }
...
.sdir-cross.left  { left: 5%; }
.sdir-cross.right { right: 5%; }
...
.sdir-orb-1 { width:260px;height:260px; top:4%; left:6%; background:rgba(201,162,39,.072); animation:floatOrb 9s ease-in-out infinite; }
.sdir-orb-2 { width:190px;height:190px; top:18%; right:8%; background:rgba(26,86,219,.055); animation:floatOrb 13s ease-in-out infinite 3.5s; }
.sdir-orb-3 { width:340px;height:340px; bottom:-90px; left:28%; background:rgba(201,162,39,.042); animation:floatOrb 18s ease-in-out infinite 7s; }
...
.sdir-orn-l.r { background:linear-gradient(270deg,transparent,rgba(201,162,39,.52)); }
.sdir-orn-ico {
    width:64px; height:64px;
    border:1.5px solid rgba(201,162,39,.44);
    border-radius:50%;
    display:flex; align-items:center; justify-content:center;
    font-size:1.7rem; color:var(--sg);
...
.sdir-ew {
    ...
    padding:6px 20px; border-radius:100px; margin-bottom:12px;
    backdrop-filter:blur(6px);
}
.sdir-ew-dot { width:6px; height:6px; border-radius:50%; background:var(--sg); animation:ewDot 2.2s ease-in-out infinite; }
...
.sdir-lbl {
    ...
    color:rgba(229,199,107,.72); margin:0 0 8px;
}
.sdir-ttl {
    font-family:var(--hf);
    font-size:clamp(3.4rem,6vw,5.6rem);
    font-weight:700; color:#f8fafc;
    margin:0 0 4px; letter-spacing:-.030em; line-height:1.02;
...
.sdir-dsc {
    font-family:var(--ff);
    font-size:clamp(1.05rem,1.8vw,1.22rem); font-style:italic; font-weight:400;
    color:rgba(226,232,240,.80);
    max-width:590px; margin:10px auto 26px;
...
.sdir-stats {
    display:inline-flex;
    border:1px solid rgba(201,162,39,.44); border-radius:100px;
...
.sdir-stat strong {
    display:block; font-size:1.26rem; font-weight:700;
...
.sdir-scroll-cue small {
    font-family:var(--bf); font-size:.58rem; letter-spacing:.14em;
    text-transform:uppercase; color:rgba(201,162,39,.38);
}
...
.sdir-scripture::after {
    ...
    font-size:.42rem; color:rgba(201,162,39,.18); pointer-events:none;
    letter-spacing:2rem;
}
...
.sdir-scripture-line.r { background:linear-gradient(270deg,transparent,rgba(201,162,39,.40)); }
.sdir-scripture-line.r::after { left:-4px; right:auto; }
.sdir-scripture-text {
    font-family:var(--ff);
    font-size:clamp(.88rem,1.4vw,1rem);
    font-style:italic; color:rgba(229,199,107,.84);
    letter-spacing:.02em; line-height:1.6;
    animation:scriptureSlide .8s ease both;
}
.sdir-scripture-text cite {
    font-family:var(--bf); font-style:normal;
    font-size:.70rem; font-weight:600;
    color:rgba(201,162,39,.52); letter-spacing:.08em; text-transform:uppercase;
    margin-left:12px;
}
...
.sdir-az-nav::-webkit-scrollbar { display:none; }
...
.sdir-az-all {
    ...
    padding:0 26px;
    font-family:var(--bf); font-size:.76rem; font-weight:700;
...
.sdir-az-all:hover { color:#fde68a; background:rgba(201,162,39,.06); text-decoration:none; }
.sdir-az-all:hover::after { width:70%; }
.sdir-az-all.active { color:#fde68a; background:rgba(201,162,39,.08); }
.sdir-az-all.active::after { width:80%; }

/* Separator */
.sdir-az-div { width:1px; background:rgba(201,162,39,.12); flex-shrink:0; margin:10px 0; }
...
.sdir-az-letter {
    ...
    gap:4px;
    font-family:var(--tf); font-size:1.06rem; font-weight:600;
...
.sdir-az-letter::before {
    content:''; position:absolute;
    inset:8px 2px; border-radius:8px;
...
.sdir-az-letter:hover::before { background:rgba(201,162,39,.09); }
.sdir-az-letter.active {
    color:#fde68a; font-weight:700;
    border-bottom-color:var(--sg);
}
.sdir-az-letter.active::before { background:rgba(201,162,39,.16); }
/* Small availability dot */
.sdir-az-dot {
    width:3px; height:3px; border-radius:50%;
    background:rgba(201,162,39,.38); flex-shrink:0;
}
.sdir-az-letter.active .sdir-az-dot { background:var(--sg); box-shadow:0 0 4px var(--sg); }
/* Unavailable */
.sdir-az-letter.off {
...
.sdir-az-letter.off .sdir-az-dot { opacity:0; }
...
.sdir-body {
    background:var(--s-bg);
    padding:52px 0 100px; min-height:60vh;
    position:relative;
}
.sdir-body::before {
    content:'';
...
.sdir-result-count {
    font-family:var(--bf); font-size:.82rem; color:var(--s-muted);
    display:flex; align-items:center; gap:6px;
}
.sdir-result-count strong { color:var(--sg); font-weight:700; font-size:.88rem; }
.sdir-clear-link {
...
.sdir-clear-link:hover {
    color:var(--sc); border-color:rgba(155,28,28,.30);
    background:rgba(155,28,28,.04); text-decoration:none;
}
...
.sdir-shl.r { background:linear-gradient(to left, rgba(201,162,39,.22), transparent); }
.sdir-sht {
    font-family:var(--tf); font-size:.98rem; font-weight:600;
...
@media(min-width:1200px){
    .sdir-featured { grid-column:span 2; }
    .sdir-featured .sdir-ciw,
    .sdir-featured .sdir-cph { height:310px; }
    .sdir-featured .sdir-img-name-text { font-size:1.62rem; line-height:1.2; }
    .sdir-featured .sdir-ttl-body { font-size:1.62rem; }
    .sdir-featured .sdir-cb { padding:24px 26px 22px; }
    .sdir-featured .sdir-cd { -webkit-line-clamp:4; font-size:.92rem; }
}
...
.sdir-card:hover::after  { animation:shimmer .70s ease forwards; }
...
.sdir-card:hover .sdir-cl { transform:scaleY(1); }
...
.sdir-card:hover .sdir-cimg { transform:scale(1.08); filter:brightness(1.10) saturate(1.14); }
...
.sdir-card:hover .sdir-cov-warm { background:rgba(201,162,39,.046); }
...
.sdir-img-name-text {
    font-family:var(--hf); font-size:1.22rem; font-weight:700;
    color:#f8fafc; line-height:1.24;
...
.sdir-cat {
    ...
    font-family:var(--bf); font-size:.60rem; font-weight:800;
...
.sdir-r1 { width:72px; height:72px; margin:-36px 0 0 -36px; animation-delay:0s; }
.sdir-r2 { width:112px; height:112px; margin:-56px 0 0 -56px; border-color:rgba(201,162,39,.08); animation-delay:.8s; }
.sdir-r3 { width:160px; height:160px; margin:-80px 0 0 -80px; border-color:rgba(201,162,39,.05); animation-delay:1.6s; }
.sdir-r4 { width:214px; height:214px; margin:-107px 0 0 -107px; border-color:rgba(201,162,39,.025); animation-delay:2.4s; }
.sdir-phx {
    position:relative; z-index:1;
    font-size:3.2rem; color:rgba(201,162,39,.42); line-height:1;
    transition:color .30s ease, transform .30s ease;
}
.sdir-card:hover .sdir-phx { color:rgba(201,162,39,.80); transform:scale(1.14); }
.sdir-phl {
    position:relative; z-index:1;
    font-family:var(--tf); font-size:.52rem;
...
.sdir-ttl-body {
    font-family:var(--hf); font-size:1.36rem; font-weight:700;
    color:var(--s-head); text-decoration:none; display:block;
    margin-bottom:10px; line-height:1.25;
    transition:color var(--tr);
}
.sdir-ttl-body:hover { color:var(--sg); text-decoration:none; }

/* Category chip in body */
.sdir-cm {
    display:inline-flex; align-items:center; gap:6px;
    background:var(--sg-soft); border:1px solid rgba(201,162,39,.18);
    color:var(--sg-h); font-family:var(--bf); font-size:.62rem; font-weight:700;
...
/* Description */
.sdir-cd {
    font-family:var(--bf); font-size:.88rem; color:var(--s-body); line-height:1.74;
...
.sdir-cf {
    display:flex; align-items:center; justify-content:space-between;
    padding-top:14px; border-top:1px solid var(--s-border); margin-top:auto;
    gap:10px;
}
...
.sdir-rl {
    ...
    color:#e5c76b;
    font-family:var(--bf); font-size:.79rem; font-weight:700;
...
.sdir-rl:hover {
    ...
    text-decoration:none; gap:11px;
}

/* Feast day badge */
.sdir-feast {
    display:inline-flex; align-items:center; gap:5px;
    font-family:var(--bf); font-size:.68rem; color:var(--s-muted);
    flex-shrink:0;
}
/* Candle icon flicker */
.sdir-feast-icon { animation:burnGlow 3.2s ease-in-out infinite; display:inline-block; }
...
.sdir-empty {
    ...
    padding:100px 40px;
    background:var(--s-card);
    border:1.5px dashed rgba(201,162,39,.22); border-radius:var(--rad);
...
.sdir-empty-ico {
    ...
    display:flex; align-items:center; justify-content:center;
    font-size:2.2rem; color:rgba(201,162,39,.55);
    margin:0 auto 22px;
    box-shadow:0 0 48px rgba(201,162,39,.18);
    animation:haloGlow 3s ease-in-out infinite;
    position:relative; z-index:1;
}
.sdir-empty h3 { font-family:var(--hf); font-size:1.55rem; color:var(--s-head); margin-bottom:10px; position:relative; z-index:1; }
.sdir-empty p  { font-family:var(--bf); font-size:.90rem; color:var(--s-muted); max-width:380px; margin:0 auto 24px; line-height:1.70; position:relative; z-index:1; }
.sdir-empty-cta {
    display:inline-flex; align-items:center; gap:8px;
    background:linear-gradient(135deg,var(--sg),#b8921e);
    color:var(--sn);
    font-family:var(--bf); font-size:.84rem; font-weight:700;
    padding:13px 30px; border-radius:100px; text-decoration:none;
    transition:all var(--tr);
    box-shadow:0 4px 18px rgba(201,162,39,.32);
    position:relative; z-index:1;
}
.sdir-empty-cta:hover { opacity:.90; transform:translateY(-2px); box-shadow:0 8px 28px rgba(201,162,39,.42); text-decoration:none; }

/* ══════════════════════════════════════════════════════════
   PAGINATION
══════════════════════════════════════════════════════════ */
.sdir-pag { display:flex; justify-content:center; margin-top:16px; position:relative; z-index:1; }

/* ══════════════════════════════════════════════════════════
   RESPONSIVE
══════════════════════════════════════════════════════════ */
@media(max-width:768px){
    .sdir-hero { padding:36px 0 28px; }
    .sdir-cross,.sdir-orb { display:none; }
    .sdir-stats { flex-direction:column; border-radius:16px; animation:none; }
    .sdir-stat { border-right:none; border-bottom:1px solid rgba(201,162,39,.12); }
    .sdir-stat:last-child { border-bottom:none; }
    .sdir-grid { grid-template-columns:repeat(auto-fill,minmax(240px,1fr)); gap:18px; }
    .sdir-scripture { padding:16px 0; }
    .sdir-scroll-cue { display:none; }
    .sdir-az-nav { min-height:54px; }
    .sdir-az-letter { font-size:.92rem; min-width:28px; }
    .sdir-az-all { padding:0 18px; font-size:.70rem; }
}
@media(max-width:480px){
    .sdir-ttl { font-size:2.6rem; }
    .sdir-grid { grid-template-columns:1fr 1fr; gap:14px; }
    .sdir-ciw,.sdir-cph { height:188px; }
    .sdir-img-name-text { font-size:1rem; }
    .sdir-cb { padding:14px 14px 12px; }
    .sdir-cd { -webkit-line-clamp:2; font-size:.82rem; }
    .sdir-rl { padding:7px 13px; font-size:.74rem; }
    .sdir-featured .sdir-ciw,
    .sdir-featured .sdir-cph { height:188px; }
}
...
<div class="sdir-az" role="navigation" aria-label="{{ __('Browse saints by letter') }}">
    ...
    <nav class="sdir-az-nav">
        ...
        <a href="{{ route('public.saints') }}"
           class="sdir-az-all @if(!$letter) active @endif"
...
<div class="container">
    ...
    <div class="sdir-result-bar">
        <div class="sdir-result-count">
            ...
            @if($letter)
                {{ __('saints under letter') }} <strong style="color:var(--s-head);font-family:var(--tf);">{{ $letter }}</strong>
            @else
        ...
        @forelse($saints as $index => $saint)
            ...
            <article class="sdir-card @if($isFeatured) sdir-featured @endif"
                ...
                <div class="sdir-cb">
                    ...
                    @if($cat)
                        <span class="sdir-cm">
                            <svg width="8" height="8" viewBox="0 0 24 24" fill="currentColor"><path d="M12 2l1.5 5h5l-4 3 1.5 5-4-3-4 3 1.5-5-4-3h5z"/></svg>
                            {{ $cat->name }}
                        </span>
                    @endif
...
(function(){
    ...
    if(el){
        ...
        if(!isNaN(num) && num >= 3){
            var cur = 0, step = Math.ceil(num / 38);
